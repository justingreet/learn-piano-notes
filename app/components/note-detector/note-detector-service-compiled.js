...
noteDetectorModule.service('noteDetectorService', ['utilService', function (utilService) {
  ...
  this.detectKeyNum = function (buffer) {
    ...
    for (var keyNum = 1; keyNum <= self.NUM_KEYS; keyNum++) {
      var curLikelihood = self.calculateLikelihoodOfFrequency(keyNum, buffer);
  ...
  self.calculateLikelihoodOfFrequency = function (keyNum, values) {
    ...
    var samplesPerPeriod = utilService.getSamplesPerPeriod(freq, self.SAMPLE_RATE);
...
//# sourceMappingURL=note-detector-service-compiled.js.map